...
def CalcCEsStep(da,mk,E,params,sta,ma,mode):
    ...
    for i,sn in enumerate(da): # i points to the action to be predicted
        if sn == 127:
            if pt_bout:
                ces_bouts += [-np.sum(np.log2(pt_bout, out=-np.full_like(pt_bout,AZERO), where=(pt_bout!=0))) / len(pt_bout)]
                pt_bout = []
                boutnum += 1
            continue
...
def CalcCEs2(da,mk,E,params,sta,ma):
    '''
    Calculate cross-entropy
    Parameters:
    da (list): data (states)
    ...
    seg=5
    ces=np.zeros(seg)
    ces_count = np.zeros(seg)
    boutnum=-1
    
    for i,sn in enumerate(da): # i points to the action to be predicted
        if sn == 127:
            if boutnum > -1 and pt:
                ces[boutnum % seg] -= np.sum(np.log2(pt, out=-np.full_like(pt,AZERO), where=(pt!=0))) / len(pt)
                ces_count[boutnum % seg] += 1
                pt = []                
...
def GetCEs2(nickname,params,ma,test=True):
    '''
    Calculate cross entropy for a specific mouse
    Parameters:
    nickname (str): nickname of the mouse
    ...
    dte=np.concatenate([b[:,0] for b in tf.no]) # test states
    mte=np.array([True]+[True,]*(len(dte)-1)) # mask for testing, all actions OK except first
    mte[np.where(dte[:-1]>62)[0]]=False # mask for testing, eliminate end nodes
    if test:
        ces = CalcCEs2(dte,mte,E,params,sta,ma,mode) # calculate cross-entropy
    else:
            ...
            for g,gamma in enumerate(gammas):
                for e,eta in enumerate(etas):
                    for d,delta in enumerate(deltas):
                        ce = GetCEs2(nickname,[beta,gamma,eta,delta],ma,mode,frame,steps) # calculate cross-entropy for this parameter combination
...
def OutCEs2(names,params,ma,fstr=None):
    '''
    Calculate and save cross-entropy for multiple mice
    Parameters:
    names (list): list of mouse nicknames
    ...
    for nickname,steps in zip(names,maxsteps):
        ...
        for b,beta in enumerate(betas):
            for g,gamma in enumerate(gammas):
                for e,eta in enumerate(etas):
                    for d,delta in enumerate(deltas):
                        ce = GetCEs2(nickname,[beta,gamma,eta,delta],ma) # calculate cross-entropy for this parameter combination
                        ces[b,g,e,d] = ce # store cross-entropy
        if fstr:
            np.save(EvalStr(fstr,nickname),ces) # save cross-entropy values to file
        ces_mult += [ces]
...
def OutCEsNamesParams2(Names,params,ma,url=None):
    ces = []
    ces_bouts = []
    for nickname,thisparams in zip(Names,params):
        ce, ce_bouts = GetCEs2(nickname,thisparams,ma,test=False)
        ces += [ce]
        ces_bouts += [ce_bouts]
    if url:
        np.save(url,np.asanyarray([ces,ces_bouts], dtype=object)) # save cross-entropy values to file
    return ces, ces_bouts
...
def GetMin2(ces,indices=()):
    ...
    for i, sublist in enumerate(ces):
        ...
        sub_min_value, sub_min_index, sub_avg_value, sub_avg_index = GetMin2(sublist, current_indices) # recursive call
...
def GetMinArgList2(Names,fstr_load,parameters,avg=False):
    '''
    Get list of parameters that minimize average cross-entropy for each mouse
    Parameters:
    Names (list): list of mouse nicknames
